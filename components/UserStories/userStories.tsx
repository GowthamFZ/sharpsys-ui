...
const UserStories = ({ review, customSlide }: { review: Stories, customSlide: any }) => {
  const { header, designation, image, content } = review;
  return (
    <div className={`rounded-3xl h-full xl:p-9 lg:p-9 pt-7.5 dark:border dark:border-strokedark dark:bg-blacksection dark:shadow-none ${customSlide === 'aboutus' ? 'lg:flex xl:flex md:flex-row w-3/4 m-auto' : 'w-full'}`}>
      <div className={`flex ${customSlide === 'aboutus' ? 'flex-row w-2/5 m-auto' : 'justify-center items-center'}`}><Image className={`rounded-full ${customSlide === 'aboutus' ? 'xl:px-4 xl:h-48 lg:px-48' : 'w-44 h-44'} `} src={image} alt={header} /> </div>
